/* $Id: GuestCtrlPrivate.cpp 115409 2017-05-15 14:30:58Z andreas.loeffler@oracle.com $ */
/** @file
        ...
        if (itGroup != mWaitEventGroups.end())
        {
            ...
            while (itEvents != itGroup->second.end())
            {
                ...
                if (RT_SUCCESS(rc2))
                {
                    ...
                    itEvents = itGroup->second.erase(itEvents);
                }
                else
                    itEvents++;
#ifdef DEBUG
        ...
        try
        {
                 ...
                 itType != lstTypes.end(); ++itType)
            {
                ...
                while (itCurEvent != mWaitEventGroups[(*itType)].end())
                {
                    ...
                    {
                        itCurEvent = mWaitEventGroups[(*itType)].erase(itCurEvent);
                        break;
                    }
                    else
                        itCurEvent++;
