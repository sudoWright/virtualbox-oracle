/* $Id: MachineImpl.cpp 57775 2010-02-16 16:57:09Z noreply@oracle.com $ */
                                       ...
                                       IN_BSTR aFlags)
{
    ...
    try
    {
        ...
        if (!mHWData->mPropertyServiceActive)
        {
            ...
            else if (SUCCEEDED(rc) && utf8Value.length())
            {
                RTTIMESPEC time;
                setModified(IsModified_MachineData);
                mHWData.backup();
                                               ...
                                               IN_BSTR aFlags)
{
    ...
    CheckComArgStrNotEmptyOrNull(aName);
    if (aValue && *aValue && (!VALID_PTR(aValue) || !VALID_PTR(aFlags)))
