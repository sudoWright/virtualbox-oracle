/* $Id: tstR0ThreadPreemption.cpp 87441 2013-07-22 14:18:41Z ramshankar.venkataraman@oracle.com $ */
/** @file
    ...
    *pszErr = '\0';
    ...
    switch (uOperation)
    {
        ...
        case TSTR0THREADPREEMPTION_CTXHOOKS:
        {
            ...
            const uint32_t  cMsTimeout           = 8000;
            const uint32_t  cMsSleepGranularity  = 50;
            uint32_t        cMsSlept             = 0;
            for (;;)
            {
                RTThreadPreemptDisable(&PreemptState);
                const RTCPUID uCurrentCpuId = RTMpCpuId();
                RTThreadPreemptRestore(&PreemptState);
