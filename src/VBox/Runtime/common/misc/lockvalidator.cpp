/* $Id: lockvalidator.cpp 56298 2009-12-31 13:21:39Z knut.osmundsen@oracle.com $ */
/** @file
 * IPRT - Lock Validator.
 */
...
{
    AssertReturn(pRec->Core.u32Magic == RTLOCKVALRECEXCL_MAGIC, NIL_RTTHREAD);
    if (!pRec->fEnabled)
        return VINF_SUCCESS;
...
{
    AssertReturnVoid(pRead->Core.u32Magic == RTLOCKVALRECSHRD_MAGIC);
    if (!pRead->fEnabled)
        return;
