/* $Id: tstDisasm-2.cpp 156593 2023-03-30 12:40:46Z alexander.eichner@oracle.com $ */
/** @file
...
static bool MyDisasIsValidInstruction(DISSTATE const *pDis)
{
    switch (pDis->pCurInstr->uOpcode)
    {
        ...
        case OP_MOV:
            if (    (   pDis->arch.x86.bOpCode == 0xc6
                     || pDis->arch.x86.bOpCode == 0xc7)
                &&  pDis->arch.x86.ModRM.Bits.Reg != 0)
                return false;
            break;
...
static DECLCALLBACK(int) MyDisasInstrRead(PDISSTATE pDis, uint8_t offInstr, uint8_t cbMinRead, uint8_t cbMaxRead)
{
                  ...
                  && pState->cbLeft >= cbMinRead))
    {
        ...
        memcpy(&pState->Dis.u.abInstr[offInstr], pState->pbNext, cbToRead);
    ...
    if (pState->uNextAddr == uSrcAddr)
    {
        ...
        if (pState->cbLeft > 0)
        {
            memcpy(&pState->Dis.u.abInstr[offInstr], pState->pbNext, pState->cbLeft);
        ...
        memset(&pState->Dis.u.abInstr[offInstr], 0xcc, cbMinRead);
        pState->rc = VERR_EOF;
    }
    else
    {
        ...
        memset(&pState->Dis.u.abInstr[offInstr], 0x90, cbMinRead);
                         ...
                         ASMSTYLE enmStyle, bool fListing, UNDEFOPHANDLING enmUndefOp)
{
    ...
    while (State.cbLeft > 0)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            uint8_t  abInstr[sizeof(State.Dis.u.abInstr)];
            memcpy(abInstr, State.Dis.u.abInstr, sizeof(State.Dis.u.abInstr));
