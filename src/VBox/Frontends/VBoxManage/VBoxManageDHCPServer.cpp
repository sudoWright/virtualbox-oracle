/* $Id: VBoxManageDHCPServer.cpp 57728 2010-02-15 17:14:18Z noreply@oracle.com $ */
/** @file
...
static int handleOp(HandlerArg *a, OPCODE enmCode, int iStart, int *pcProcessed)
{
    ...
    if(enmCode == OP_ADD)
    {
        ...
        CHECK_ERROR(a->virtualBox, CreateDHCPServer(NetName.raw(), svr.asOutParam()));
        if(FAILED(rc))
    ...
    if(enmCode != OP_REMOVE)
    {
        ...
        {
            CHECK_ERROR(svr, SetConfiguration(Bstr(pIp).raw(), Bstr(pNetmask), Bstr(pLowerIp), Bstr(pUpperIp)));
            if(FAILED(rc))
